...
def collect_args() -> argparse.Namespace:
    ...
    parser.add_argument('-i', '--input', type=str, default='orders.csv',
                        help="Order data")
    parser.add_argument('-c', '--candles', type=str, default='candles.csv',
                        help="Candle data dump")
    parser.add_argument('-o', '--output', type=str, default='data.json',
                        help="Output charting json")
...
async def gen_candles(start_time: int, end_time: int) -> list:
    ...
    async with ClientSession() as session:
        chunks = map(lambda url: asyncio.ensure_future(fetch_url(url, session)),
                     urls)
...
def parse_orders(filename) -> dict:
    ...
    for _, trade in df.iterrows():
        ...
        orders.append(exit)
    return orders, df.entry_time.min(), df.exit_time.max()


async def main():
   args = collect_args()

   orders, start_time, end_time = parse_orders(args.input)
